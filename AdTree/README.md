...
- **Note:** In order for the python modules to find the build executable, you should set the correct path in [`config.py`](../src/tree_modeling/config.py). When using the commands above, this will be the default setting (i.e. `../../AdTree/bin/AdTree`).
